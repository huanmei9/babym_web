...
void change_dvr_conf(std::string old_mode, std::string new_mode) {
    std::ifstream instream("conf/pc.conf");
    std::ofstream outstream("conf/pc.conf.copy");
    std::string str;
	size_t pos = 0;
	...
	while (std::getline(instream, str)) {
		...
		if (pos != std::string::npos) {
			...
			pos = str.find(old_mode);
            if(pos != std::string::npos) {
                str.replace(pos, old_mode.length(), new_mode);
            }
			outstream << str << std::endl;
			continue;
		}
    ...
    std::remove("conf/pc.conf");
    std::rename("conf/pc.conf.copy", "conf/pc.conf");
}
...
srs_error_t SrsGoApiRaw::serve_http(ISrsHttpResponseWriter* w, ISrsHttpMessage* r)
{
    ...
    // Custom: for rpc=send, trigger server to run python scripy to send flv file
    if (rpc == "send") {
        // change conf file
        change_dvr_conf("off", "on");
        system("./scripts/send.sh");
        obj->set("code", SrsJsonAny::integer(1));
        // obj->set("response", SrsJsonAny::str("Test!"));

        return srs_api_response(w, r, obj->dumps());
    }

    if(rpc == "stop") {
        change_dvr_conf("on", "off");
        system("./scripts/stop.sh");
        obj->set("code", SrsJsonAny::integer(1));
    ...
    if(rpc == "query") {
        ...
        std::ifstream infile;
        infile.open("./python/result.txt");
        if(infile.good()) {
            std::string message;
            infile >> message;
            obj->set("code", SrsJsonAny::integer(0));
            obj->set("response", SrsJsonAny::str(message.c_str()));    
        }else {
            // file not exist
            obj->set("code", SrsJsonAny::integer(-1));
            obj->set("response", SrsJsonAny::str("message file not exist!"));
        }
        infile.close();
        return srs_api_response(w, r, obj->dumps());
    }
